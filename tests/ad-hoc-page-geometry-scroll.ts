...
async function testContinuousScrollGeometry(pdfPath: string, passes: number = 5): Promise<void> {
  console.log("  Continuous scroll geometry test with " + passes + ' passes on "' + pdfPath + '"...');
  const results = await withControlledSumatra(
    EXE,
    async (client) => {
      const res = await client.request(ControlCommand.TestPageGeometry, [pdfPath, passes]);
  ...
  if (exitCode !== 0) {
    ...
    throw new Error("Continuous scroll geometry test FAILED: geometry mismatch");
  }
...
async function testZoomGeometryStability(pdfPath: string, cycles: number = 6): Promise<void> {
  console.log("  Zoom stability test: " + cycles + " open/render/verify cycles...");
  for (let i = 0; i < cycles; i++) {
    const results = await withControlledSumatra(
      EXE,
      async (client) => {
        const res = await client.request(ControlCommand.TestPageGeometry, [pdfPath, 1]);
        const exitCode = res[0] as number;
        const raw = String(res[1] ?? "").trim();
        return { exitCode, raw };
      },
...
async function testInterleavedPageAccess(pdfPath: string, iterations: number = 8): Promise<void> {
  console.log("  Interleaved access stress: " + iterations + " iterations...");
  for (let i = 0; i < iterations; i++) {
    const results = await withControlledSumatra(
      EXE,
      async (client) => {
        const res = await client.request(ControlCommand.TestPageGeometry, [pdfPath, 2]);
        const exitCode = res[0] as number;
        const raw = String(res[1] ?? "").trim();
        return { exitCode, raw };
      },
