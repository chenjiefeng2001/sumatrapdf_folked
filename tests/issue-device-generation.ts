...
async function testParallelDeviceGenerationStress(): Promise<void> {
  ...
  const tasks: Promise<void>[] = [
    runControlCommand(EXE, ControlCommand.TestPageLinks, [PDF, 1]).then((res) => {
      const r = String(res[0] ?? "").trim();
      if (r.includes("ERROR")) throw new Error("parallel render [1]: " + r);
    }),
    runControlCommand(EXE, ControlCommand.TestPageLinks, [PDF, 2]).then((res) => {
      const r = String(res[0] ?? "").trim();
      if (r.includes("ERROR")) throw new Error("parallel render [2]: " + r);
    }),
    runControlCommand(EXE, ControlCommand.TestPageLinks, [PDF, 3]).then((res) => {
      const r = String(res[0] ?? "").trim();
      if (r.includes("ERROR")) throw new Error("parallel render [3]: " + r);
    }),
    runControlCommand(EXE, ControlCommand.TestGetToc, [PDF]).then((res) => {
      const r = String(res[0] ?? "").trim();
      if (r.includes("ERROR")) throw new Error("parallel TOC: " + r);
    }),
    runControlCommand(EXE, ControlCommand.TestDest, [PDF, 1]).then((res) => {
      const r = String(res[0] ?? "").trim();
      if (r.includes("ERROR")) throw new Error("parallel dest: " + r);
    }),
    runControlCommand(EXE, ControlCommand.TestDest, [PDF, 2]).then((res) => {
      const r = String(res[0] ?? "").trim();
      if (r.includes("ERROR")) throw new Error("parallel dest 2: " + r);
    }),
  ];
