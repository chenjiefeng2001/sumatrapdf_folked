...
static void HwndListAdd(HwndBase* w) {
    // Guard against double-registration: the same object may arrive via
    // WM_NCCREATE (custom window class) and/or Subclass() (standard controls).
    // Skip silently if already registered instead of removing + re-adding.
    for (auto& e : gHwndToWnd) {
        if (e.wnd == w) {
            return;
        }
...
void SizeToIdealSize(ControlBase* c) {
    ...
    c->SetBounds(Rect{pt.x, pt.y, size.dx, size.dy});
}
