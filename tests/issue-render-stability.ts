...
async function testParallelStress(): Promise<void> {
  ...
  const tasks: Promise<void>[] = [
    runControlCommand(EXE, ControlCommand.TestPageLinks, [PDF, 1]).then((r) => {
      const v = String(r[0] ?? "").trim();
      if (v.includes("ERROR")) throw new Error("p1: " + v);
    }),
    runControlCommand(EXE, ControlCommand.TestPageLinks, [PDF, 2]).then((r) => {
      const v = String(r[0] ?? "").trim();
      if (v.includes("ERROR")) throw new Error("p2: " + v);
    }),
    runControlCommand(EXE, ControlCommand.TestPageLinks, [PDF, 3]).then((r) => {
      const v = String(r[0] ?? "").trim();
      if (v.includes("ERROR")) throw new Error("p3: " + v);
    }),
    runControlCommand(EXE, ControlCommand.TestPageLinks, [PDF, 4]).then((r) => {
      const v = String(r[0] ?? "").trim();
      if (v.includes("ERROR")) throw new Error("p4: " + v);
    }),
    runControlCommand(EXE, ControlCommand.TestPageLinks, [PDF, 5]).then((r) => {
      const v = String(r[0] ?? "").trim();
      if (v.includes("ERROR")) throw new Error("p5: " + v);
    }),
    runControlCommand(EXE, ControlCommand.TestGetToc, [PDF]).then((r) => {
      const v = String(r[0] ?? "").trim();
      if (v.includes("ERROR")) throw new Error("toc: " + v);
    }),
    runControlCommand(EXE, ControlCommand.TestDest, [PDF, 1]).then((r) => {
      const v = String(r[0] ?? "").trim();
      if (v.includes("ERROR")) throw new Error("d1: " + v);
    }),
    runControlCommand(EXE, ControlCommand.TestSearch, [PDF, "Page", 0]).then((r) => {
      const v = String(r[0] ?? "").trim();
      if (v.includes("ERROR")) throw new Error("search: " + v);
    }),
  ];
