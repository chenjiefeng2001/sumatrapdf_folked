...
async function testMixedZoomScrollParallel(): Promise<void> {
  ...
  const tasks: Promise<void>[] = [
    runControlCommand(EXE, ControlCommand.TestPageLinks, [PDF, 1]).then((res) => {
      ...
      if (r.includes("ERROR")) throw new Error("parallel zoom [1]: " + r);
    }),
    runControlCommand(EXE, ControlCommand.TestPageLinks, [PDF, 2]).then((res) => {
      const r = String(res[0] ?? "").trim();
      if (r.includes("ERROR")) throw new Error("parallel scroll [2]: " + r);
    }),
    runControlCommand(EXE, ControlCommand.TestPageLinks, [PDF, 3]).then((res) => {
      const r = String(res[0] ?? "").trim();
      if (r.includes("ERROR")) throw new Error("parallel scroll [3]: " + r);
    }),
    runControlCommand(EXE, ControlCommand.TestDest, [PDF, 1]).then((res) => {
      const r = String(res[0] ?? "").trim();
      if (r.includes("ERROR")) throw new Error("parallel dest [1]: " + r);
    }),
    runControlCommand(EXE, ControlCommand.TestSearch, [PDF, "Page", 0]).then((res) => {
      const r = String(res[0] ?? "").trim();
      if (r.includes("ERROR")) throw new Error("parallel search: " + r);
    }),
  ];
